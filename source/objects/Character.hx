	...
	public function loadCharacterFile(json:Dynamic) {
		...
		if (json.animateZip != null) {
			...
			if (FileSystem.exists(zipPath)) {
				...
				// IMPORTANT: ZIP characters still use offsets and metadata
				// so continue loading JSON metadata normally!
			} else
				trace("Animate ZIP not found: " + zipPath);
		}

		//
		// -----------------------------------------------------
		// Normal FlxAnimate multi-atlas support
		// -----------------------------------------------------
		//
		#if flxanimate
		...
		//
		// -----------------------------------------------------
		// Only load frames if NOT ZIP atlas
		// -----------------------------------------------------
		//
		if (!isAnimateZIP) {
			scale.set(1, 1);
		...
		//
		// -----------------------------------------------------
		// Load all JSON metadata (needed for ZIP or PNG)
		// -----------------------------------------------------
		//
		...
		// Register animation names and offsets
		if (animationsArray != null) {
			for (anim in animationsArray) {
				var name = anim.anim;
				addOffset(name, anim.offsets != null ? anim.offsets[0] : 0, anim.offsets != null ? anim.offsets[1] : 0);
			}
		}
