    ...
    function autoRegisterAnimateAnimations():Void {
        ...
        if (character.isAnimateAtlas && character.atlas != null && character.atlas.anim != null) {            
            ...
            if (animObj != null)
            {
                ...
                keys = cast Reflect.fields(animObj.animsMap);
            ...
            // Very old version animList:Array<String> = [];
        ...
        // ------------------------------------------------------------
        // 2. Animate Folder (data.json + library.json)
        // ------------------------------------------------------------
        if (character.isAnimateFolder && character.animateLibrary != null) {
            ...
            for (key in Reflect.fields(dict)) {
                ...
                if (symbol == null || symbol.className == null || symbol.className == "")
                continue;
    ...
    override function update(elapsed:Float) {
        ...
        if (!character.isAnimationNull()) {
            ...
            if (character.isAnimateAtlas && character.atlas != null && character.atlas.anim != null)
            {
                ...
                if (Reflect.hasField(animObj, "animsMap"))
                {
                    for (key in Reflect.fields(animObj.animsMap))
                    names.push(key);
                }
                else if (Reflect.hasField(animObj, "nameMap"))
                {
