...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	/**
	 * Collect all animations from Animate Atlas or Animate Folder.
	 * Adds them to character.animationsArray if not already present.
	 */
	function autoRegisterAnimateAnimations():Void {
		// Animate Atlas: Animation.json
		#if flxanimate
		if (character.isAnimateAtlas && character.atlas != null && character.atlas.anim != null) {
			// ------------------------------------------------------------
			// Collect animations from Animate Atlas
			// ------------------------------------------------------------
			var animList:Iterable<String> = null;

			// Newer flxanimate API
			if (character.atlas.anim != null && Reflect.hasField(character.atlas.anim, "getAnimationList")) {
				animList = cast character.atlas.anim.getAnimationList();
			}
			// Mid-version API
			else if (Reflect.hasField(character.atlas, "animationList")) {
				animList = cast Reflect.field(character.atlas, "animationList");
			}
			// Very old API used by early Dot-Stuff builds
			else if (Reflect.hasField(character.atlas.anim, "animList")) {
				animList = cast Reflect.field(character.atlas.anim, "animList");
			}

			// Fallback just to prevent crashes
			if (animList == null)
				animList = [];
			...
			for (name in animList) {
				...
				var symName:String = name;

				if (!character.animationsArray.exists(a -> a.anim == symName)) {
					character.animationsArray.push({
						anim: symName,
						name: symName,
						fps: 24,
						loop: false,
				...
				for (key in dict.keys()) {
					...
					if (symbol.className != null && symbol.className != "") {
						...
						if (!Lambda.exists(character.animationsArray, a -> a.anim == symName)) {
							...
							character.addOffset(symName, 0, 0);
						}
					}
				}
			}
		...
		function addHelpScreen() {
			var str:Array<String> = [
				...
				"W/S - Previous/Next Animation", "Space - Replay Animation", "Arrow Keys/Mouse & Right Click - Move Offset",
		...
		function addCharacter(reload:Bool = false) {
			...
			// NEW — Automatically add Animate symbols as animations
			autoRegisterAnimateAnimations();
		...
		function reloadCharacterImage() {
			...
			if (Paths.fileExists('images/' + character.imageFile + '/Animation.json', TEXT)) {
				...
				try {
					Paths.loadAnimateAtlas(character.atlas, character.imageFile, true);
				} catch (e:Dynamic) {
