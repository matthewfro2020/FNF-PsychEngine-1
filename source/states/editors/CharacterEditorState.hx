...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	function reloadAnimationDropDown() {
		// ================================================
		// SAFE animList generation (v7)
		// ================================================
		var animList:Array<String> = [];

		if (character != null) {
			// PNG / MultiAtlas animations
			for (a in character.animationsArray)
				animList.push(a.anim);

			// AnimateAtlas animations (safe)
			#if flxanimate
			if (character.isAnimateAtlas && character.atlas != null && character.atlas.anim != null) {
				var fieldNames = Reflect.fields(character.atlas.anim);
	...
	// [AUTOPATCH_EDITOR]
	#if flxanimate
	// Allow Animate characters to list all symbols in editor:
	function listAnimateSymbols(char:Character) {
		if (!char.isAnimateAtlas)
			return [];

		var keys = [];

		var map:Map<String, Dynamic> = cast char.atlas.anim.symbolMap;
		for (k => v in map)
			keys.push(k);
